...
if not path_obj.exists():
    raise Exception('Please make sure the path to eynsham_dataset dataset is correct')

if not path_obj.joinpath('queries_night'):
    raise Exception(
        f'Please make sure the directory train_val from eynsham_dataset dataset is situated in the directory {DATASET_ROOT}')
...
class SvoxnightDataset(Dataset):
    def __init__(self, input_transform=None):
        self.input_transform = input_transform

        # hard coded reference image names, this avoids the hassle of listing them at each epoch.
        self.dbImages = np.load(
            'datasets/svox_night/svox_night_val_dbImages.npy')

        # hard coded query image names.
        self.qImages = np.load('datasets/svox_night/svox_night_val_qImages.npy')

        # hard coded index of query images
        self.qIdx = np.load('datasets/svox_night/svox_night_val_qIdx.npy')

        # hard coded groundtruth (correspondence between each query and its matches)
        self.pIdx = np.load('datasets/svox_night/svox_night_val_pIdx.npy',
                            allow_pickle=True)
    ...
    def __getitem__(self, index):
        ...
        # 如果是单通道图像，将其转换为三通道
        if img.mode == 'L':
            print(f"Warning: {self.images[index]} is a single channel image. Converting to RGB.")
