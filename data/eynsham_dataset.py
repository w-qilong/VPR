...
class EynshamDataset(Dataset):
    def __init__(self, input_transform=None):
        ...
        self.dbImages = np.load(
            'datasets/eynsham/eynsham_val_dbImages.npy')

        # hard coded query image names.
        self.qImages = np.load('datasets/eynsham/eynsham_val_qImages.npy')

        # hard coded index of query images
        self.qIdx = np.load('datasets/eynsham/eynsham_val_qIdx.npy')

        # hard coded groundtruth (correspondence between each query and its matches)
        self.pIdx = np.load('datasets/eynsham/eynsham_val_pIdx.npy',
                            allow_pickle=True)
