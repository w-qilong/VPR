...
if not path_obj.exists():
    raise Exception('Please make sure the path to GardensPointWalking_dataset dataset is correct')

if not path_obj.joinpath('query'):
    raise Exception(
        f'Please make sure the directory train_val from GardensPointWalking_dataset dataset is situated in the directory {DATASET_ROOT}')
...
class GardenspointDataset(Dataset):
    def __init__(self, input_transform=None):
        ...
        self.dbImages = np.load(
            'datasets/GardensPointWalking/GardensPoint_walking_val_dbImages.npy')

        # hard coded query image names.
        self.qImages = np.load('datasets/GardensPointWalking/GardensPoint_walking_val_qImages.npy')

        # hard coded index of query images
        self.qIdx = np.load('datasets/GardensPointWalking/GardensPoint_walking_val_qIdx.npy')

        # hard coded groundtruth (correspondence between each query and its matches)
        self.pIdx = np.load('datasets/GardensPointWalking/GardensPoint_walking_val_pIdx.npy',
                            allow_pickle=True)
