...
if not path_obj.exists():
    raise Exception('Please make sure the path to st_lucia_dataset dataset is correct')

if not path_obj.joinpath('query'):
    raise Exception(
        f'Please make sure the directory train_val from st_lucia_dataset dataset is situated in the directory {DATASET_ROOT}')
...
class StluciaDataset(Dataset):
    def __init__(self, input_transform=None):
        ...
        self.dbImages = np.load(
            'datasets/st_lucia/st_lucia_val_dbImages.npy')

        # hard coded query image names.
        self.qImages = np.load('datasets/st_lucia/st_lucia_val_qImages.npy')

        # hard coded index of query images
        self.qIdx = np.load('datasets/st_lucia/st_lucia_val_qIdx.npy')

        # hard coded groundtruth (correspondence between each query and its matches)
        self.pIdx = np.load('datasets/st_lucia/st_lucia_val_pIdx.npy',
                            allow_pickle=True)
