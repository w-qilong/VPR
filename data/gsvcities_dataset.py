...
BASE_PATH = r'/media/cartolab3/DataDisk/wuqilong_file/VPR_datasets/GSV-Cities/'
...
class GsvcitiesDataset(Dataset):
    def __init__(self,
                 cities=[
                     ...
                     'BuenosAires',
                     'LosAngeles',
                     'MexicoCity', 'OSL', 'Rome', 'Barcelona', 'Chicago',
                     'Madrid', 'Miami', 'Phoenix', 'TRT', 'Boston', 'Lisbon', 'Medellin', 'Minneapolis',
                     'PRG', 'WashingtonDC', 'Brussels',
                     'London',
                     'Melbourne',
                     'Osaka',
                     'PRS'
        ...
        '''
        :param cities: which city's name used for training
        :param img_per_place: number of images for each place
        :param min_img_per_place: min number of images for each place
        :param random_sample_from_each_place: whether select images for each place randomly
        :param transform: the transforms used for images
        :param base_path: the root folder of dataset
        '''
...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(dataset, batch_size=64, drop_last=False, pin_memory=True, shuffle=False, num_workers=10)
    step = 0
    for epoch in range(50):
        for index, place_id in enumerate(dataloader):
            step += 1
            if  4800 <= step < 5800:
                print(place_id)

        # place_id = batch[1][0][0]
        # print(place_id)
