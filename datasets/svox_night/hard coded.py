...
DATASET_ROOT = r'/media/cartolab3/DataDisk/wuqilong_file/VPR_datasets/svox/images/test'
path_obj = Path(DATASET_ROOT)

if not path_obj.exists():
    raise Exception('Please make sure the path to svox_night dataset is correct')

if not path_obj.joinpath('queries_night'):
    raise Exception(
        f'Please make sure the directory train_val from svox_night dataset is situated in the directory {DATASET_ROOT}')
else:
    queries_folder = path_obj.joinpath('queries_night')

if not path_obj.joinpath('gallery'):
    raise Exception(
        f'Please make sure the directory train_val from svox_night dataset is situated in the directory {DATASET_ROOT}')
else:
    database_folder = path_obj.joinpath('gallery')
...
# hard code
np.save(os.path.join(current_file_dir, 'svox_night_val_dbImages.npy'),database_paths)
np.save(os.path.join(current_file_dir, 'svox_night_val_qImages.npy'),queries_paths)
np.save(os.path.join(current_file_dir, 'svox_night_val_qIdx.npy'),qIdx)
...
# 保存numpy数组到文件
np.save(os.path.join(current_file_dir, 'svox_night_val_pIdx.npy'),irregular_array)

# 加载pIdx
# 从文件中加载numpy数组
irregular_array = np.load(os.path.join(current_file_dir, 'svox_night_val_pIdx.npy'), allow_pickle=True)
