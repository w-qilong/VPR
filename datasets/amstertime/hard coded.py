...
if not path_obj.exists():
    raise Exception('Please make sure the path to amstertime dataset is correct')

if not path_obj.joinpath('queries'):
    raise Exception(
        f'Please make sure the directory train_val from amstertime dataset is situated in the directory {DATASET_ROOT}')
else:
...
# hard code
np.save(os.path.join(current_file_dir, 'amstertime_val_dbImages.npy'),database_paths)
np.save(os.path.join(current_file_dir, 'amstertime_val_qImages.npy'),queries_paths)
np.save(os.path.join(current_file_dir, 'amstertime_val_qIdx.npy'),qIdx)
...
# 保存numpy数组到文件
np.save(os.path.join(current_file_dir, 'amstertime_val_pIdx.npy'),irregular_array)

# 加载pIdx
# 从文件中加载numpy数组
irregular_array = np.load(os.path.join(current_file_dir, 'amstertime_val_pIdx.npy'), allow_pickle=True)
