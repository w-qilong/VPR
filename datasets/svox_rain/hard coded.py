...
if not path_obj.joinpath('queries_rain'):
    raise Exception(
        f'Please make sure the directory train_val from svox_rain dataset is situated in the directory {DATASET_ROOT}')
else:
    queries_folder = path_obj.joinpath('queries_rain')
...
# hard code
np.save(os.path.join(current_file_dir, 'svox_rain_val_dbImages.npy'),database_paths)
np.save(os.path.join(current_file_dir, 'svox_rain_val_qImages.npy'),queries_paths)
np.save(os.path.join(current_file_dir, 'svox_rain_val_qIdx.npy'),qIdx)
...
# 保存numpy数组到文件
np.save(os.path.join(current_file_dir, 'svox_rain_val_pIdx.npy'),irregular_array)

# 加载pIdx
# 从文件中加载numpy数组
irregular_array = np.load(os.path.join(current_file_dir, 'svox_rain_val_pIdx.npy'), allow_pickle=True)
