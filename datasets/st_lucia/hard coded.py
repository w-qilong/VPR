...
DATASET_ROOT = r'/media/cartolab3/DataDisk/wuqilong_file/VPR_datasets/st_lucia/images/test'
...
if not path_obj.joinpath('query'):
    raise Exception(
        f'Please make sure the directory train_val from st_lucia dataset is situated in the directory {DATASET_ROOT}')
else:
...
# hard code
np.save(os.path.join(current_file_dir, 'st_lucia_val_dbImages.npy'),database_paths)
np.save(os.path.join(current_file_dir, 'st_lucia_val_qImages.npy'),queries_paths)
np.save(os.path.join(current_file_dir, 'st_lucia_val_qIdx.npy'),qIdx)
...
# 保存numpy数组到文件
np.save(os.path.join(current_file_dir, 'st_lucia_val_pIdx.npy'),irregular_array)

# 加载pIdx
# 从文件中加载numpy数组
irregular_array = np.load(os.path.join(current_file_dir, 'st_lucia_val_pIdx.npy'), allow_pickle=True)
