...
# from utils import chunk_cosine_sim
# from losses import compute_guided_matching
from .hook_func import (
...
def get_correspondences(
    ...
    patch_indices = torch.arange(total_patches, device=device)[None].expand(
        query_desc.shape[0], -1
    )  # shape: [B, N]

    best_buddies_mask = (
        torch.gather(nn_r2q, 1, nn_q2r) == patch_indices
    )  # shape: [B, N]
    best_buddies_mask = torch.bitwise_and(best_buddies_mask, sim_mask)  # shape: [B, N]

    # 扩展前景掩码到批次维度
    ref_fg_mapped = torch.gather(
        nn_r2q, 1, torch.where(ref_fg_mask)[1][None].expand(query_desc.shape[0], -1)
    )
    ref_fg_mask_mapped = torch.zeros(
        (query_desc.shape[0], total_patches), dtype=torch.bool, device=device
    )
...
def get_rerank_results(
    ...
    for index, pre in enumerate(first_predictions):
        ...
        # 重排序
        cur_ref_topk_local_features = torch.tensor(
            cur_ref_topk_local_features
        ).cuda()  # 50 128 64
        cur_query_topk_local_features = (
            torch.tensor(cur_query_topk_local_features)
            .unsqueeze(0)
            .repeat(cur_ref_topk_local_features.shape[0], 1, 1)
            .cuda()
        )  # 128 64
        # 计算引导匹配
        with torch.no_grad():
