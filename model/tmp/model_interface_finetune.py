...
class AggMInterface(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        cls_token, normalized_CNN_DINO_fused_features = self.forward(
            images
        )
        ...
        # 使用permute重新排列维度顺序
        a1_fused_features = normalized_CNN_DINO_fused_features[a1].permute(0, 2, 3, 1)  # B*C*H*W -> B*H*W*C
        p_fused_features = normalized_CNN_DINO_fused_features[p].permute(0, 2, 3, 1)    # B*C*H*W -> B*H*W*C
        a2_fused_features = normalized_CNN_DINO_fused_features[a2].permute(0, 2, 3, 1)  # B*C*H*W -> B*H*W*C
        n_fused_features = normalized_CNN_DINO_fused_features[n].permute(0, 2, 3, 1)    # B*C*H*W -> B*H*W*C

        # 检查是否有有效的样本对
        if len(a1) == 0 or len(a2) == 0:
            # 如果没有有效的样本对，直接返回metric loss
            return {"loss": metric_loss}
        
        # 计算local loss时分批处理以节省显存
        local_batch_size = 8  # 可以根据显存大小调整此值
        ...
        # 分批计算a1和p之间的local loss
        for i in range(0, len(a1), local_batch_size):
            batch_a1 = a1_fused_features[i:i + local_batch_size]
            batch_p = p_fused_features[i:i + local_batch_size]
            
            # 计算当前批次的loss
            curr_a1p_loss = self.local_loss_function(batch_a1, batch_p)
            total_a1p_loss += torch.mean(curr_a1p_loss).item()  # 只保存数值
            num_a1p_batches += 1
        ...
        # 分批计算a2和n之间的local loss
        for i in range(0, len(a2), local_batch_size):
            batch_a2 = a2_fused_features[i:i + local_batch_size]
            batch_n = n_fused_features[i:i + local_batch_size]
            
            # 计算当前批次的loss
            curr_a2n_loss = self.local_loss_function(batch_a2, batch_n)
            total_a2n_loss += torch.mean(curr_a2n_loss).item()  # 只保存数值
            num_a2n_batches += 1
        ...
        # 计算平均loss（确保不会除以0）
        avg_a1p_loss = total_a1p_loss / num_a1p_batches if num_a1p_batches > 0 else 0
        avg_a2n_loss = total_a2n_loss / num_a2n_batches if num_a2n_batches > 0 else 0
        
        # 计算总的local loss
        total_local_loss = torch.tensor(max(-avg_a1p_loss + avg_a2n_loss + 0.0, 0.0),
                                    device=metric_loss.device)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        places, _ = batch
        # 计算描述符
        cls_token, normalized_CNN_DINO_fused_features = self.forward(places)
        # 保存每个数据加载器的每个batch输出
