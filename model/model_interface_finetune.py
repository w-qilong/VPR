...
class AggMInterface(pl.LightningModule):
    ...
    def configure_loss(self):
        ...
        if self.metric_loss_function in [
            "MultiSimilarityLoss",
            "TripletMarginLoss",
            "ContrastiveLoss",
            "NCALoss",
    ...
    def on_train_start(self):
        ...
        if self.hparams.save_feats:
            ...
            self.tmp_imgs = [
                Image.open(img_path).convert("RGB") for img_path in self.img_paths
            ]
            ...
            if self.hparams.rerank:
                ...
                for saliency_thresh in saliency_threshs:
                    for nn_match_thresh in nn_match_threshs:
                        ...
                        print(
                            f"saliency_thresh: {np.round(saliency_thresh, 2)}, nn_match_thresh: {np.round(nn_match_thresh, 2)}"
                        )
                        ...
                        for k in k_values:
                            self.log(
                                f"{val_set_name}_{saliency_thresh}_{nn_match_thresh}_rerank/R{k}",
                                d[k],
                                prog_bar=False,
                                logger=True,
                            )
                        ...
                        if self.hparams.rerank:
                            ...
                            save_path = os.path.join(
                                self.trainer.log_dir, "second_predictions.xlsx"
                            )
                            df = pd.DataFrame.from_dict(
                                self.val_rerank_results, orient="index"
                            )
    ...
    def find_improved_examples(
        ...
        for q_idx, (first_pred, rerank_pred) in enumerate(
            zip(first_predictions, rerank_predictions)
        ):
            # 统计重排序前和重排序后前top_k中正确结果的数量
            first_correct_count = np.sum(np.in1d(first_pred[:top_k], positives[q_idx]))
            rerank_correct_count = np.sum(
                np.in1d(rerank_pred[:top_k], positives[q_idx])
            )
            ...
            if rerank_correct_count > first_correct_count:
                ...
                query_path = (
                    val_dataset.get_image_path(query_idx)
                    if hasattr(val_dataset, "get_image_path")
                    else str(query_idx)
                )
                ...
                for idx in first_pred[:top_k]:
                    path = (
                        val_dataset.get_image_path(idx)
                        if hasattr(val_dataset, "get_image_path")
                        else str(idx)
                    )
                ...
                example = {
                    ...
                    "positives": (
                        positives[q_idx].tolist()
                        if isinstance(positives[q_idx], np.ndarray)
                        else positives[q_idx]
                    ),
    ...
    def visualize_improved_examples(
        self, val_dataset, num_references, improved_examples, save_dir
    ):
        """
        ...
        for idx, example in enumerate(improved_examples):
            ...
            with open(examples_info_file, "a") as f:
                ...
                for i, path in enumerate(example.get("first_pred_paths", [])):
                    f.write(
                        f"  Top {i+1}: {path} {'(correct)' if first_preds[i] in positives else ''}\n"
                    )
                f.write("Reranked predictions:\n")
                for i, path in enumerate(example.get("rerank_pred_paths", [])):
                    f.write(
                        f"  Top {i+1}: {path} {'(correct)' if rerank_preds[i] in positives else ''}\n"
                    )
                f.write(
                    f"Improvement: {first_correct} -> {rerank_correct} correct matches\n\n"
                )
            ...
            plt.savefig(
                os.path.join(save_dir, f"example_{idx}.png"),
                bbox_inches="tight",
                facecolor="white",
                dpi=600,
            )
